...
Response OrdersController::List(const RequestContext& ctx) const {
  int limit = ctx.query.TryGet<int>("limit").value_or(20);
  int offset = ctx.query.TryGet<int>("offset").value_or(0);
  std::string_view offset_strv = ctx.query.TryGet<std::string_view>("offset").value_or("0");
  auto offset_str = ctx.query.TryGet<std::string>("offset").value_or("0");
  auto offset_bool = ctx.query.TryGet<bool>("offset").value_or(false);
...
Response OrdersController::GetOrderName(const RequestContext& ctx) const {
  ...
  bool random = ctx.query.TryGet<bool>("random").value_or(false);
